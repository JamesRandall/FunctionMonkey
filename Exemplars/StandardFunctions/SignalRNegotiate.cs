...
{
    public static class SignalRConnectionInfoAttributeExtensions
    {
        public static IEnumerable<Claim> GetClaims(this SignalRConnectionInfoAttribute attribute)
        {
            List<Claim> claimList = new List<Claim>();
            if (!string.IsNullOrEmpty(attribute.UserId))
                claimList.Add(new Claim("http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier", attribute.UserId));
    ...
    public class AzureSignalRClient
    {
        ...
        public AzureSignalRClient(string connectionString)
        {
            ValueTuple<string, string, string> connectionString1 = AzureSignalRClient.ParseConnectionString(connectionString);
