...
namespace AzureFromTheTrenches.Commanding.AzureFunctions.Compiler.Implementation
{
    internal class TemplateProvider : ITemplateProvider
    {
        ...
        {
            using (Stream stream = GetType().Assembly.GetManifestResourceStream("AzureFromTheTrenches.Commanding.AzureFunctions.Compiler.Templates.proxies.json.handlebars"))
        ...
        private string GetTemplate(AbstractFunctionDefinition functionDefinition, string type)
        {
            ...
            {
                using (Stream stream = GetType().Assembly.GetManifestResourceStream($"AzureFromTheTrenches.Commanding.AzureFunctions.Compiler.Templates.{prefix}.{type}.handlebars"))
                using (StreamReader reader = new StreamReader(stream))
                    return reader.ReadToEnd();
            }
